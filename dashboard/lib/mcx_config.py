...
# Reads MCX_SUPABASE_URL / MCX_SUPABASE_KEY so this co-exists with NSE/BSE
# Supabase env (SUPABASE_URL / SUPABASE_KEY) in the same project. Module-level
# constants keep their original names to avoid touching downstream imports.
SUPABASE_URL = os.environ.get("MCX_SUPABASE_URL", "https://avqwpebveqetwwzkmtux.supabase.co")
SUPABASE_ANON_KEY = os.environ.get("MCX_SUPABASE_KEY", "")

# ─── CORS (F-13: restricted to deployment domains) ───────────────────────────
ALLOWED_ORIGINS = os.environ.get(
    "ALLOWED_ORIGINS",
    "https://exchange-pipeline.vercel.app,https://mcx-vercel.vercel.app,http://localhost:8765,http://localhost:3000"
).split(",")
