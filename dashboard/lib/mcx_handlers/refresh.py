...
if not callable(globals().get("supabase_read")):
    _SB_URL = os.environ.get("MCX_SUPABASE_URL", "https://avqwpebveqetwwzkmtux.supabase.co")
    _SB_KEY = os.environ.get("MCX_SUPABASE_KEY", "")
    ...
    def do_GET(self):
        ...
        try:
            ...
            if rows:
                ...
                # ── Staleness detection ────────────────────────────────
                captured_str = row.get("captured_at", "")
                is_stale = False
                snapshot_age_min = 0
                if captured_str:
                    try:
                        captured_dt = datetime.fromisoformat(captured_str.replace("Z", "+00:00"))
                        age_sec = (datetime.now(timezone.utc) - captured_dt).total_seconds()
                        snapshot_age_min = round(age_sec / 60, 1)
                        is_stale = snapshot_age_min > 5
                    except Exception:
                        pass

                if is_stale:
                    result["is_stale"] = True
                    result["snapshot_age_min"] = snapshot_age_min
                    result["stale_warning"] = f"Relay data is {int(snapshot_age_min)} min old"
                    # Re-project with current elapsed if session has advanced
                    now = now_ist()
                    cur_elapsed = max(0, min(now.hour * 60 + now.minute - SESSION_START, SESSION_TOTAL))
                    if cur_elapsed > elapsed:
                        pf, po, conf = project_full_day(fut_n, opt_p, cur_elapsed, day_type)
                        result["proj_fut_cr"] = round(pf, 2)
                        result["proj_opt_cr"] = round(po, 2)
                        _, _, _, total_rev = calc_revenue(pf, po)
                        result["proj_total_rev"] = round(total_rev, 4)
                        result["confidence"] = conf
                        cur_pct = cur_elapsed / SESSION_TOTAL if SESSION_TOTAL > 0 else 0
                        unc = calc_uncertainty(cur_pct, day_type, dual)
                        result["uncertainty_pct"] = round(unc * 100, 1)
                        result["rev_low"] = round(total_rev * (1 - unc), 2)
                        result["rev_high"] = round(total_rev * (1 + unc), 2)
                        result["elapsed_min"] = cur_elapsed
                        result["elapsed_pct"] = round(cur_elapsed / SESSION_TOTAL * 100, 1)
                else:
